...
public class PurchaseAndReturnFrame extends JInternalFrame {
	...
	public PurchaseAndReturnFrame() {
		...
		sp.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				InventoryInfo info = (InventoryInfo) sp.getSelectedItem();
				// 如果选择有效就更新表格
		...
		tjButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				for (int i = 0; i < table.getRowCount(); i++) {
					InventoryInfo info = (InventoryInfo) table.getValueAt(i, 0);
		...
		rkButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (jsrStr == null || jsrStr.isEmpty()) {
					JOptionPane.showMessageDialog(PurchaseAndReturnFrame.this, "请填写经手人");
					return;
				}
				if (ysjlStr == null || ysjlStr.isEmpty()) {
					JOptionPane.showMessageDialog(PurchaseAndReturnFrame.this, "填写验收结论");
					return;
				}
				if (table.getRowCount() <= 0) {
					JOptionPane.showMessageDialog(PurchaseAndReturnFrame.this, "填加退货商品");
					return;
				}
				WarhosRtMainInfo rkthMain = new WarhosRtMainInfo(id, pzsStr, jeStr, ysjlStr,gysName, rkDate, czyStr, jsrStr, jsfsStr);
				Set<WarhosRtDetailInfo> set = rkthMain.getTbRkthDetails();
				int rows = table.getRowCount();
				for (int i = 0; i < rows; i++) {
					InventoryInfo kucun = (InventoryInfo) table.getValueAt(i, 0);
					String djStr = (String) table.getValueAt(i, 6);
					String slStr = (String) table.getValueAt(i, 7);
					Double dj = Double.valueOf(djStr);
					Integer sl = Integer.valueOf(slStr);
					WarhosRtDetailInfo detail = new WarhosRtDetailInfo();
				...
				boolean rs = DBOper.insertRkthInfo(rkthMain);
				if (rs) {
					JOptionPane.showMessageDialog(PurchaseAndReturnFrame.this, "退货完成");
	...
	private void initSpBox() {
		...
		ResultSet set = DBOper.query("select * from tb_kucun where "
				+ "id in(select id from tb_spinfo where gysName='"
				+ gys.getSelectedItem() + "')");
		sp.removeAllItems();
		sp.addItem(new InventoryInfo());
		for (int i = 0; table != null && i < table.getRowCount(); i++) {
			InventoryInfo tmpInfo = (InventoryInfo) table.getValueAt(i, 0);
	...
	private void doGysSelectAction() {
		...
		SupplierInfo gysInfo = DBOper.getGysInfo(item);
	...
	private final class computeInfo implements ContainerListener {
		public void componentRemoved(ContainerEvent e) {
			...
			double money = 0.0;
			// 计算品种数量
			InventoryInfo column = null;
			if (rows > 0)
				column = (InventoryInfo) table.getValueAt(rows - 1, 0);
	...
	private final class initTasks extends InternalFrameAdapter {
		...
		private void initGysField() {// 初始化供应商字段
			List gysInfos = DBOper.getGysInfos();
	...
	private void initPiaoHao() {
		...
		String maxId = DBOper.getRkthMainMaxId(date);
		piaoHao.setText(maxId);
	}
	// 根据商品下拉框的选择，更新表格当前行的内容
	private synchronized void updateTable() {
		InventoryInfo kucun = (InventoryInfo) sp.getSelectedItem();
