...
public class CommodityQueryFrame extends JInternalFrame{
	...
	private void updateTable(List list, final DefaultTableModel dftm) {
		...
		CommodityInfo spInfo;
		while (iterator.hasNext()) {
			List info = (List) iterator.next();
			Item item = new Item();
			item.setId((String) info.get(0));
			item.setName((String) info.get(1));
			spInfo = DBOper.getSpInfo(item);
		...
		private List searchInfo(String conName, String conOperation,
			...
			if (conOperation.equals("等于")) {
				if (conName.equals("商品名称"))
					list = DBOper.findForList(sql + "spname='" + content + "'");
				if (conName.equals("供应商全称"))
					list = DBOper.findForList(sql + "gysname='" + content + "'");
				if (conName.equals("产地"))
					list = DBOper.findForList(sql + "cd='" + content + "'");
				if (conName.equals("规格"))
					list = DBOper.findForList(sql + "gg='" + content + "'");
			} else {
				if (conName.equals("商品名称"))
					list = DBOper.findForList(sql + "spname like '%" + content
							+ "%'");
				if (conName.equals("供应商全称"))
					list = DBOper.findForList(sql + "gysname like '%" + content
							+ "%'");
				if (conName.equals("产地"))
					list = DBOper.findForList(sql + "cd like '%" + content + "%'");
				if (conName.equals("规格"))
					list = DBOper.findForList(sql + "gg like '%" + content + "%'");
			}
			return list;
		}
	}
