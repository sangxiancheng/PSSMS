...
public class ImportBillFrame extends JInternalFrame{
	...
	private void initSpBox() {
		...
		ResultSet set = DBOper.query("select * from tb_spinfo where gysName='"
				+ gys.getSelectedItem() + "'");
		sp.removeAllItems();
	...
	class RkActionListener implements ActionListener {	// 入库按钮的事件监听器
		public void actionPerformed(ActionEvent e) {
			...
			if (jsrStr == null || jsrStr.isEmpty()) {
				JOptionPane.showMessageDialog(ImportBillFrame.this, "请填写经手人");
				return;
			}
			if (ysjlStr == null || ysjlStr.isEmpty()) {
				JOptionPane.showMessageDialog(ImportBillFrame.this, "填写验收结论");
				return;
			}
			if (table.getRowCount() <= 0) {
				JOptionPane.showMessageDialog(ImportBillFrame.this, "填加入库商品");
				return;
			}
			WarehouseMainInfo ruMain = new WarehouseMainInfo(id, pzsStr, jeStr, ysjlStr,gysName, rkDate, czyStr, jsrStr, jsfsStr);
			Set<WarehouseDetailInfo> set = ruMain.getTabRukuDetails();
			...
			for (int i = 0; i < rows; i++) {
				...
				WarehouseDetailInfo detail = new WarehouseDetailInfo();
			...
			boolean rs = DBOper.insertRukuInfo(ruMain);
			if (rs) {
				JOptionPane.showMessageDialog(ImportBillFrame.this, "入库完成");
	...
	private void initPiaoHao() {
		...
		String maxId = DBOper.getRuKuMainMaxId(date);
