...
public class PowerManageFrame extends JInternalFrame {
	...
	public PowerManageFrame() {
		...
		closeButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				PowerManageFrame.this.doDefaultCloseAction();
			}
		});
		// 处理修改按钮的单击事件
		modifyButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				UserInfo user = DBOper.getUser(item);
				int index = quanXian.getSelectedIndex();
				if (index == 0)
					user.setQuan("a");
				else
					user.setQuan("c");
				if (DBOper.updateUser(user) >= 1)
					JOptionPane.showMessageDialog(PowerManageFrame.this, "修改完成");
				else
					JOptionPane.showMessageDialog(PowerManageFrame.this, "修改失败");
			}
		});
		// 窗体激活事件
	...
	private void doUserSelectAction() {
		...
		UserInfo user = DBOper.getUser(selectedItem);
