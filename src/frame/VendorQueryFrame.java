...
public class VendorQueryFrame extends JInternalFrame {
	...
	private void updateTable(List list, final DefaultTableModel dftm) {
		...
		SupplierInfo gysInfo;
		while (iterator.hasNext()) {
			List info = (List) iterator.next();
			Item item = new Item();
			item.setId((String) info.get(0));
			item.setName((String) info.get(1));
			gysInfo = DBOper.getGysInfo(item);
		...
		public void actionPerformed(final ActionEvent e) {
			...
			if (conName.equals("供应商全称")) {
				if (conOperation.equals("等于"))
					list = DBOper.findForList(sql + "name='" + content + "'");
				else
					list = DBOper.findForList(sql + "name like '%" + content
							+ "%'");
			} else {
				if (conOperation.equals("等于"))
					list = DBOper.findForList(sql + "jc='" + content + "'");
				else
					list = DBOper.findForList(sql + "jc like '%" + content + "%'");
			}
			updateTable(list, dftm);
		}
	}
