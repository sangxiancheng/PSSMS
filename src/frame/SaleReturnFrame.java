...
public class SaleReturnFrame extends JInternalFrame{
		...
		thButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (jsrStr == null || jsrStr.isEmpty()) {
					JOptionPane.showMessageDialog(SaleReturnFrame.this, "请填写经手人");
					return;
				}
				if (ysjlStr == null || ysjlStr.isEmpty()) {
					JOptionPane.showMessageDialog(SaleReturnFrame.this, "填写验收结论");
					return;
				}
				if (table.getRowCount() <= 0) {
					JOptionPane.showMessageDialog(SaleReturnFrame.this, "填加退货商品");
					return;
				}
				SaleReturnMainInfo xsthMain = new SaleReturnMainInfo(id, pzsStr, jeStr,ysjlStr, gysName, rkDate, czyStr, jsrStr, jsfsStr);
				Set<SaleReturnDetailInfo> set = xsthMain.getTbXsthDetails();
				...
				for (int i = 0; i < rows; i++) {
					...
					SaleReturnDetailInfo detail = new SaleReturnDetailInfo();
				...
				boolean rs = DBOper.insertXsthInfo(xsthMain);
				if (rs) {
					JOptionPane.showMessageDialog(SaleReturnFrame.this, "退货完成");
	...
	private void initPiaoHao() {
		...
		String maxId = DBOper.getXsthMainMaxId(date);
