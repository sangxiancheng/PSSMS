...
public class SaleQueryFrame extends JInternalFrame {
	...
	class ShowAllActoinListener implements ActionListener {			// 显示全部按钮的动作监听器
		public void actionPerformed(final ActionEvent e) {
			content.setText("");
			List list=DBOper.findForList("select * from v_sellView");
	...
	class QueryActionListener implements ActionListener {
		public void actionPerformed(final ActionEvent e) {
			...
			list = DBOper.findForList("select * from v_sellView where "
					+ con
					+ opstr
					+ (oper == 0 ? "'"+cont+"'" : "'%" + cont + "%'")
