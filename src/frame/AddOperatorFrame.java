...
public class AddOperatorFrame extends JPanel {
	...
	public AddOperatorFrame() {
		...
		button.addActionListener(new ActionListener() {
			public void actionPerformed(final ActionEvent e) {
				...
				UserInfo user = DBOper.getUser(tbName, password);
				if (user.getUsername() != null && !user.getUsername().isEmpty()) {
					JOptionPane.showMessageDialog(AddOperatorFrame.this,"此登录名称已经存在");
					name.setFocusable(true);
					return;
				}
				if (name.getText() == null || name.getText().equals("")) {
					JOptionPane.showMessageDialog(AddOperatorFrame.this, "用户名不能为空");
					name.setRequestFocusEnabled(true);
					return;
				}
				if (passwordField.getPassword().length == 0) {
					JOptionPane.showMessageDialog(AddOperatorFrame.this, "密码不能为空");
				} else if (passwordField.getPassword().equals(passwordField_1.getPassword())) {
					UserInfo tbUserlist = new UserInfo();
					tbUserlist.setQuan("c");
					tbUserlist.setPass(String.valueOf(passwordField_1.getPassword()));
					tbUserlist.setName(logName.getText().trim());
					tbUserlist.setUsername(name.getText().trim());
					DBOper.addUser(tbUserlist);
					JOptionPane.showMessageDialog(AddOperatorFrame.this, "操作员添加成功");
					clear();
				} else {
					JOptionPane.showMessageDialog(AddOperatorFrame.this, "两次密码输入不相同");
				}
			}
		});
		button.setText("添加");
