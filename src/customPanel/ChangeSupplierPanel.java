	...
	class ModifyActionListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			...
			if (DBOper.updateGys(gysInfo) == 1)
				JOptionPane.showMessageDialog(ChangeSupplierPanel.this, "修改完成");
			else
				JOptionPane.showMessageDialog(ChangeSupplierPanel.this, "修改失败");
		}
	}
	...
	class DelActionListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			...
			if (item == null || !(item instanceof Item))
				return;
			int confirm = JOptionPane.showConfirmDialog(ChangeSupplierPanel.this, "确认删除供应商信息吗？");
			if (confirm == JOptionPane.YES_OPTION) {
				int rs = DBOper.delete("delete tb_gysInfo where id='"+ item.getId() + "'");
				if (rs > 0) {
					JOptionPane.showMessageDialog(ChangeSupplierPanel.this,"供应商：" + item.getName() + "。删除成功");
					gys.removeItem(item);
				} else {
					JOptionPane.showMessageDialog(ChangeSupplierPanel.this,"无法删除客户：" + item.getName() + "。");
				}
			}
		}
	}
