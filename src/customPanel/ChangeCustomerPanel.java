...
public class ChangeCustomerPanel extends JPanel{
	...
	public ChangeCustomerPanel() {
		...
		delButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (item == null || !(item instanceof Item))
					return;
				int confirm = JOptionPane.showConfirmDialog(ChangeCustomerPanel.this, "确认删除客户信息吗？");
				if (confirm == JOptionPane.YES_OPTION) {
					int rs = DBOper.delete("delete tb_khinfo where id='"+ item.getId() + "'");
					if (rs > 0) {
						JOptionPane.showMessageDialog(ChangeCustomerPanel.this,"客户：" + item.getName() + "。删除成功");
		...
		modifyButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				Item item = (Item) kehu.getSelectedItem();
				CustomerInfo khinfo = new CustomerInfo();
				...
				if (DBOper.updateKeHu(khinfo) == 1)
					JOptionPane.showMessageDialog(ChangeCustomerPanel.this, "修改完成");
				else
					JOptionPane.showMessageDialog(ChangeCustomerPanel.this, "修改失败");
			}
		});
	}
	// 初始化客户下拉选择框
	public void initComboBox() {
		List khInfo = DBOper.getKhInfos();
	...
	private void doKeHuSelectAction() {
		...
		CustomerInfo khInfo = DBOper.getKhInfo(selectedItem);
