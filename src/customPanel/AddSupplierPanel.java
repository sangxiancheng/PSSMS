	...
	class TjActionListener implements ActionListener {// 添加按钮的事件监听类
		public void actionPerformed(final ActionEvent e) {
			if (diZhiF.getText().equals("") || quanChengF.getText().equals("")
				...
				JOptionPane.showMessageDialog(AddSupplierPanel.this, "请填写全部信息");
				return;
			}
			try {
				ResultSet haveUser = DBOper.query("select * from tb_gysinfo where name='"+ quanChengF.getText().trim() + "'");
				if (haveUser.next()) {
					JOptionPane.showMessageDialog(AddSupplierPanel.this,"供应商信息添加失败，存在同名供应商", "供应商添加信息",JOptionPane.INFORMATION_MESSAGE);
					return;
				}
				ResultSet set = DBOper.query("select max(id) from tb_gysinfo");
				String id = null;
				...
				DBOper.addGys(gysInfo);
				JOptionPane.showMessageDialog(AddSupplierPanel.this, "已成功添加客户","客户添加信息", JOptionPane.INFORMATION_MESSAGE);
				resetButton.doClick();
